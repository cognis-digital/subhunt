"""Native cognis-connect emit for subhunt — forward findings to any platform.

Maps subhunt's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    subhunt ... --format json | subhunt-emit --to stix
    subhunt-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('host', '')
        out['severity'] = 'info'
        out['type'] = 'subdomain'
        out['description'] = f"Subdomain found: {rec.get('host', '')}"
